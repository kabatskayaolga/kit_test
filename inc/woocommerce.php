...
// 	wp_enqueue_style( 'theme-woocommerce-style', get_template_directory_uri() . '/woocommerce.css' );
	...
	function theme_woocommerce_cart_link() {
		?>
			<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'theme' ); ?>">
				...
				<span class="amount"><?php echo wp_kses_data( WC()->cart->get_cart_subtotal() ); ?></span> <span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'theme' ), WC()->cart->get_cart_contents_count() ) );?></span>
			</a>
		<?php
	}
}
...
add_filter( 'loop_shop_per_page', create_function( '$cols', 'return 10;' ), 10 );
